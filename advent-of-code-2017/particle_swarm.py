...
class Particle(object):
    ...
    def __init__(self, key, particle_raw):
        self.key = key

        position_split = particle_raw.split('p=<')[1].split('>, ')[0].split(',')
        self.position = (int(position_split[0]), int(position_split[1]), int(position_split[2]))

        velocity_split = particle_raw.split('v=<')[1].split('>, ')[0].split(',')
        self.velocity = (int(velocity_split[0]), int(velocity_split[1]), int(velocity_split[2]))

        accel_split = particle_raw.split('a=<')[1].split('>')[0].split(',')
        self.acceleration = (int(accel_split[0]), int(accel_split[1]), int(accel_split[2]))
    ...
    def get_pos_accel_eval(self):
        return abs(self.position[0] + self.acceleration[0]) + abs(self.position[1] + self.acceleration[1]) \
            + abs(self.position[2] + self.acceleration[2])

    def get_pos_vel_eval(self):
        return abs(self.position[0] + self.velocity[0]) + abs(self.position[1] + self.velocity[1]) \
            + abs(self.position[2] + self.velocity[2])

    def get_vel_accel_eval(self):
        return abs(self.velocity[0] + self.acceleration[0]) + abs(self.velocity[1] + self.acceleration[1]) \
            + abs(self.velocity[2] + self.acceleration[2])

    def get_manhattan_acceleration(self):
        return abs(self.acceleration[0]) + abs(self.acceleration[1]) + abs(self.acceleration[2])

    def get_manhattan_velocity(self):
...
class ParticleSwarm(object):
    ...
    def __init__(self, particles_raw):
        particles_split = particles_raw.split('\n')

        curr_key = 0
        for par in particles_split:
            self.particles.append(Particle(curr_key, par))
            curr_key += 1

    def get_closest_ad_infinitum(self):
        if len(self.particles) == 0:
            return 0

        # Step 1: Find the minimum of manhattan accelerations
        print 'Finding minimum manhattan acceleration'

        min_list = self.get_min_manhattan_acceleration(self.particles)

        # Step 2: Find the minimum of velocity-acceleration evaluators amongst the remainders
        if len(min_list) == 1:
            return min_list[0].key
        else:
            print 'Tie detected. Finding minimum velocity-acceleration evaluator'

        min_list = self.get_min_vel_accel_evaluator(min_list)

        # Step 3: Find the minimum of position-acceleration evaluators amongst the remainders
        if len(min_list) == 1:
            return min_list[0].key
        else:
            print 'Tie detected. Finding minimum position-acceleration evaluator'

        min_list = self.get_min_pos_accel_evaluator(min_list)

        # Step 4: Find the minimum of position-velocity evaluators amongst the remainders
        if len(min_list) == 1:
            return min_list[0].key
        else:
            print 'Tie detected. Finding minimum position-velocity evaluator'

        min_list = self.get_min_pos_vel_evaluator(min_list)
    ...
    def get_num_sans_collisions(self):
        # Sort the particles in order of manhattan acceleration
        particles_sorted = self.sort_manhattan_acceleration()

        i = 0
        # While ad infinitum, move particles, filter collisions, print minimum
        while len(particles_sorted) > 1:
            position_hash = {}

            # move
            for particle in particles_sorted:
                particle.move()
                hash_key = str(particle.position)
                if hash_key in position_hash:
                    position_hash[hash_key].append(particle.key)
                else:
                    position_hash[hash_key] = [particle.key]
      
            # filter
            # print position_hash[str(particles_sorted[0].position)]
            prev_len = len(particles_sorted)
            particles_sorted = [p for p in particles_sorted if str(p.position) not in position_hash or len(position_hash[str(p.position)]) <= 1]
            if len(particles_sorted) < prev_len:
                print len(particles_sorted)
            # print minimum
            # print particles_sorted[0].key
            i += 1
            if i == 1000:
                break


        return len(particles_sorted)


    def sort_manhattan_acceleration(self):
        return sorted(self.particles, key=lambda particle: particle.get_manhattan_acceleration())

    def get_min_manhattan_acceleration(self, prev_list):
        min_list = [prev_list[0]]
        min_accel = prev_list[0].get_manhattan_acceleration()

        # While loop so no need for generating a full range list, or reiterating index 0
        i = 1
        while i < len(prev_list):
            curr_accel = prev_list[i].get_manhattan_acceleration()
            if curr_accel < min_accel:
                min_list = [prev_list[i]]
                min_accel = curr_accel
            elif curr_accel == min_accel:
                min_list.append(prev_list[i])

            i += 1
        
        return min_list

    def get_min_vel_accel_evaluator(self, prev_list):
        min_list = [prev_list[0]]
        min_eval = prev_list[0].get_vel_accel_eval()

        # While loop so no need for generating a full range list, or reiterating index 0
        i = 1
        while i < len(prev_list):
            curr_eval = prev_list[i].get_vel_accel_eval()
    ...
    def get_min_pos_accel_evaluator(self, prev_list):
        min_list = [prev_list[0]]
        min_eval = prev_list[0].get_pos_accel_eval()

        # While loop so no need for generating a full range list, or reiterating index 0
        i = 1
        while i < len(prev_list):
            curr_eval = prev_list[i].get_pos_accel_eval()
            if curr_eval < min_eval:
                min_list = [prev_list[i]]
                min_eval = curr_eval
            elif curr_eval == min_eval:
    ...
    def get_min_pos_vel_evaluator(self, prev_list):
        min_list = [prev_list[0]]
        min_eval = prev_list[0].get_pos_vel_eval()

        # While loop so no need for generating a full range list, or reiterating index 0
        i = 1
        while i < len(prev_list):
            curr_eval = prev_list[i].get_pos_vel_eval()
            if curr_eval < min_eval:
                min_list = [prev_list[i]]
                min_eval = curr_eval
            elif curr_eval == min_eval:
...
pset_input = """p=<-1724,-1700,5620>, v=<44,-10,-107>, a=<2,6,-9>
...
ps = ParticleSwarm(pset_input)
print ps.get_num_sans_collisions()