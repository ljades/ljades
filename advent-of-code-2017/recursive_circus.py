...
class RecursiveCircus(object):
    ...
    def set_program_list(self, program_list_raw):
        program_list_parsed = {}

        # parse the input
        for program in program_list_raw:
            p_name = program.split(' ')[0]
            p_children_split = program.split(' -> ')
            p_children = {}
            if len(p_children_split) > 1:
                p_children_list = p_children_split[1].split(', ')
                for child in p_children_list:
                    p_children[child] = 1
            program_list_parsed[p_name] = { '': int(program.split('(')[1].split(')')[0]), 'children': p_children }

        self.program_list = program_list_parsed


    def find_root(self):
        # get the first key/program
        key = self.program_list.keys()[0]

        # while the current key is known to have a parent, find the parent and set the key equal to that
        while True:
            key_has_parents = False

            for target_key in self.program_list:
                if target_key == key:
                    continue

                if key in self.program_list[target_key]['children']:
                    key_has_parents = True
                    key = target_key
                    break

            if not key_has_parents:
                break

        return key

    def calculate_totals(self, root):
        children_sum = 0
        for child in self.program_list[root]['children']:
            children_sum += self.calculate_totals(child)
        
        self.program_list[root]['total'] = self.program_list[root][''] + children_sum
        return self.program_list[root]['total']

    def get_fixed_wrong_weight(self, program_list_raw):
        self.set_program_list(program_list_raw)
        root = self.find_root()
        self.calculate_totals(root)
        return self.fixed_wrong_weight_recursive(root, -1)


    def fixed_wrong_weight_recursive(self, root, deficit):
        num_children = len(self.program_list[root]['children'])
        child_fixed = -1
        wrong_child = ''
        # Figure out which child is the one where, either in its weight or in its children, there is a problem
        # Return -1 if no balance issues here
        if num_children == 0:
            return -1
        elif num_children == 1:
            wrong_child = self.program_list[root]['children'].keys()[0]
        elif num_children == 2:
            child_1 = self.program_list[root]['children'].keys()[0]
            child_2 = self.program_list[root]['children'].keys()[1]
            if deficit != -1:
                if self.program_list[child_1]['total'] + deficit == self.program_list[child_2]['total']:
                    wrong_child = child_1
                elif self.program_list[child_2]['total'] + deficit == self.program_list[child_1]['total']:
                    wrong_child = child_2
                else:
                    return -1
        elif num_children > 2:
            wrong_child = self.get_wrong_child_from_multiple(self.program_list[root]['children'].keys())
            if wrong_child == '':
                return -1

        # Final exception case where num_children == 2 and deficit == -1
        if num_children == 2 and deficit == -1:
            fixed_wrong_weight_1 = self.fixed_wrong_weight_recursive(child_1, deficit)
            fixed_wrong_weight_2 = self.fixed_wrong_weight_recursive(child_2, deficit)

        if deficit == -1:
            wrong_total = self.program_list[wrong_child]['total']
            for child in self.program_list[root]['children']:
                if self.program_list[child]['total'] != wrong_total:
                    right_total = self.program_list[child]['total']
            deficit = right_total - wrong_total
        
        # Recursively correct it, get the fixed wrong weight
        fixed_wrong_weight = self.fixed_wrong_weight_recursive(wrong_child, deficit)
        # if there's a deficit and nothing wrong with the children, 
        if fixed_wrong_weight == -1:
            return self.program_list[wrong_child][''] + deficit
        else:
            return fixed_wrong_weight

        
        

    # 
    def get_wrong_child_from_multiple(self, children_keys):
        child_vals = {}
        wrong_total = 0
        for child in children_keys:
            if self.program_list[child]['total'] in child_vals:
                child_vals[self.program_list[child]['total']] += 1
            else:
                child_vals[self.program_list[child]['total']] = 1
        ...
        if len(child_vals) == 1:
            ...
            for total in child_vals.keys(): # find the total with only 1, which means that the other ones are equal and there can't be anything wrong
                if child_vals[total] == 1:
                    wrong_total = total
        else:
...
print rc.get_fixed_wrong_weight(pset_input)

print rc.program_list['hxmcaoy']