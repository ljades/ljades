...
class RegisterStore(object):
    ...
    def greater_than(registers, register, val):
        return registers[register] > val

    def greater_than_or_equal(registers, register, val):
        return registers[register] >= val

    def equal(registers, register, val):
        return registers[register] == val

    def not_equal(registers, register, val):
        return registers[register] != val

    def less_than_or_equal(registers, register, val):
        return registers[register] <= val

    def less_than(registers, register, val):
        return registers[register] < val

    def add_to_register(registers, register, val):
        registers[register] += val
        return

    def sub_to_register(registers, register, val):
        registers[register] -= val
        return
    ...
    def largest_value_register(self, instructions):
        for instruction in instructions:
            self.interpret_single_instruction(instruction)

        return self.get_max_register()

    def interpret_single_instruction(self, instruction):
        instruction_split = instruction.split(' ')
        operation_register = instruction_split[0]
        if operation_register not in self.registers:
            self.registers[operation_register] = 0
        operation = instruction_split[1]
        operation_val = int(instruction_split[2])
        condition_register = instruction_split[4]
        if condition_register not in self.registers:
            self.registers[condition_register] = 0
        conditional = instruction_split[5]
        condition_val = int(instruction_split[6])

        if self.operators[conditional](self.registers, condition_register, condition_val):
            self.operators[operation](self.registers, operation_register, operation_val)
        
        if self.registers[operation_register] > self.max_ever_held:
            self.max_ever_held = self.registers[operation_register]

        print self.max_ever_held
        return

    def get_max_register(self):
        max_register = self.registers.keys()[0]
        max_value = self.registers[max_register]

        for register in self.registers:
            if self.registers[register] > max_value:
                max_value = self.registers[register]
                max_register = register
        return max_register


pset_input = """av inc 167 if f > -9
av inc 640 if uea == 0
fk dec -960 if tn > -9
...
reg = RegisterStore()
print reg.largest_value_register(pset_input)
